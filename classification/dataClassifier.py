...
def enhancedPacmanFeatures(state, action):
    ...
    features = util.Counter()
    "*** YOUR CODE HERE ***"
    oldPacmanPosition = state.getPacmanPosition()
    newState = state.generatePacmanSuccessor(action)
    newPacmanPosition = newState.getPacmanPosition()
    #newPosHasFood
    if state.hasFood(newPacmanPosition[0], newPacmanPosition[1]):
        features['newPosHasFood'] = 1
    else:
        features['newPosHasFood'] = 0
    state = state.generatePacmanSuccessor(action)
    pacmanPosition = state.getPacmanPosition()
    ghostPositions = state.getGhostPositions()
    score = state.getScore()
    pelletPositions = state.getCapsules()
    foodCoordinates = state.getFood().asList()
...
##        dist = util.manhattanDistance(food, pacmanPosition)
##        if dist < leastDistance:
##            leastDistance = dist
##    features['closestFood'] = leastDistance
    
##    #pelletCount
##    features['pelletCount'] = len(pelletPositions)

    
    #closestPellet
    leastDistance = float('inf')
    for pellet in pelletPositions:
        dist = util.manhattanDistance(pellet, pacmanPosition)
        if dist < leastDistance:
            leastDistance = dist
            features['closestPellet'] = leastDistance
    ...
    for ghost in ghostPositions:
        ...
        if dist < leastDistance:
            leastDistance = dist
    features['closestGhost'] = leastDistance
...
##
    #closestScaredGhost
    leastDistance = float('inf')
    for i in range(len(ghostScaredTimes)):
        if ghostScaredTimes[i] != 0:
            dist = util.manhattanDistance(ghostPositions[i], pacmanPosition)
            if dist < leastDistance:
                leastDistance = dist
                features['closestScaredGhost'] = leastDistance
