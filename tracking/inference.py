...
##        self.ghostOldPos = util.Counter()
##        self.ghostOldPos[self.index] = gameState.getGhostPosition(self.index)
##        print self.ghostOldPos
...
##        util.raiseNotDefined()
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        #print self.ghostAgent.position#getPosition()
##        print gameState.getNumAgents()#getGhostPosition(self.index)
##        newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, self.ghostOldPos[self.index]))
##        print newPosDist
##        print self.beliefs
##        print gameState.getGhostPosition(self.index)

        '''
        P(ghost in new position n)  = summation over 'o' (P(o, n))
                                    = summation over 'o' (P(n|o) * P(o))
        P(o) is the previous self.beliefs i.e. self.beliefs[o]
        where 'o' is old position of the ghost
        P(n|o) is given by newPosDist in the following code
        '''
        allPossible = util.Counter()
...
##                    allPossible[p] *= emissionModels[i][trueDistance]
            ...
            for k in range(self.numGhosts):
                if noisyDistances[k] == None:
                    for l in range(len(self.particles)):
                        self.particles[l] = self.getParticleWithGhostInJail(self.particles[l], k)
        else:
            allPossible.normalize()
            for m in range(len(self.particles)):
                self.particles[m] = util.sample(allPossible)
